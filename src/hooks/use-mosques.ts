...
export const mosquesKeys = {
  ...
  listAll: (filters?: Omit<MosqueFilters, 'page' | 'perPage'>) => [...mosquesKeys.lists(), 'all', filters] as const,
...
export const useMosquesAll = (filters?: Omit<MosqueFilters, 'page' | 'perPage'>) => {
...
export const useUpdateMosque = () => {
  ...
  return useMutation({
    mutationFn: ({ id, data, imageFile, deleteImage }: { id: string; data: Partial<Mosque>; imageFile?: File; deleteImage?: boolean }) => 
